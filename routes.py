...
def complete():
    ...
    # no hay problema que no exista para una fecha , se genera automáticamente 
    # dado que es un diccionario predeterminado
    current_app.db.completions.insert_one({"date":date , "habit":habit})

    return redirect(url_for(".index",date=date_string))#currrent blue print por eso el .index en lugar de habits.index